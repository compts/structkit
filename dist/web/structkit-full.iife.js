...
function lastIndexOf (objectValue, value) {
    ...
    var indexValue = getIndexOf(objectValue, value, zero, count(objectValue), true);
...
function pSerialize (value) {
    ...
    return curryArg(function (rawValue) {
        ...
        if (indexOfExist([
            ...
            var mapData = map(range(count(rawValue) - one, zero), function (mValue, kValue) {
...
function getObjectValue (value) {
    ...
    var splitClose = arraySlice(splitOpen, one).join("{")
        .replace(/\}[;]{1,}$/g, "");
    ...
    if (getMatch.is_valid) {
        ...
        if (splitValue[zero] === "a") {
            ...
            each(range(convertValue(splitValue[one]) - one, zero), function () {
                ...
                var refobjVal = parseTypeValObj(arraySlice(objValue, one).join(";")+";");
...
function templateValue (templateString, data, option) {
    ...
    return curryArg(function (rawTemplateString, rawData, rawOption) {
        ...
        var sourceData = reduce("", rawData, function (total, vv, kk) {
